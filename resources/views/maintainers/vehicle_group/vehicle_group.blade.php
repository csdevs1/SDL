        ...
        <nav class="navbar" role="navigation">
            <div class="container-fluid">
                <div class="navbar-header">
                    ...
                    <span class="navbar-brand">Nuevo Grupo de Veh&#237;culos</span>
                </div>
...
<?php } ?>
          ...
          <div class="modal-body">
              <div class="panel panel-default">
                  <div class="panel-body panel-body-form">
                      ...
                      <div class="form-group">
                          <label>Asociar Vehiclos</label>
                          <select class="form-control" id="vehicles" multiple>
                              <?php foreach($vehicles as $key=>$val){ ?>
                                  <option class="option_vehicles" value="<?php echo $val->id; ?>"><?php echo $val->plate; ?></option>
                              <?php } ?>
                          </select>
                      </div>
                  </div>
    ...
    <script>
        // UPDATE
            var update_group = function(arr,id,vehicles){
            ...
            $(document).on("click","#update",function() { // Button update in modal
               var group_label=$('#edit-label').val(),
                ...
                if(Object.keys(arr).length>=1 && group_id!=''){
                    var update=update_group(arr,group_id,vehicles);
        ...
        // SAVE
            var save_group = function(arr){
                ...
                return $.ajax({
                    ...
                    contentType:  false,
                    data:formData,
                    url: '/maintainers/create/vehicle_groups'
                });
            }
            $(document).on("click","#save",function() {
                ...
                if(error.length<=0){
                    ...
                    create.done(function(r){
                        if(Object.keys(r).length>0){
                            $('<tr id="'+r['id']+'"><td class="counter"><span class="badge">0</span></td><td class="label-val">'+r['label']+'</td><td><span class="">'+r['description']+'</span></td><td><span class="badge">0</span></td><td><span class="badge">'+r['created']+'</span></td><td><span class="label label-success action-btn edit" data-id="'+r['id']+'" data-label="'+r['label']+'" data-description="'+r['description']+'" data-toggle="modal" data-target="#editModal">Editar</span><span class="label label-danger action-btn delete" data-id="'+r['id']+'" data-label="'+r['label']+'">Eliminar</span></td></tr>').prependTo("table > tbody").hide().fadeIn('slow');
                            $('#name').val('');
                            $('#description').val('');
                            ...
                            $('#snackbar').text('Grupo de Vehiculo guardado correctamente!');
